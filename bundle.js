;(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);throw new Error("Cannot find module '"+o+"'")}var f=n[o]={exports:{}};t[o][0].call(f.exports,function(e){var n=t[o][1][e];return s(n?n:e)},f,f.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
	...
	//require("./delaunay.js")
	// put everything into one .js file and then run 
	//browserify animation.js > bundle.js
	//in shell
	// everything must be in one single.js file (pput it there using require, ie as the first two lines above)
	
	//var a = require("./enlargeObstacles.js");
	//var b = require("./motionPlanning.js");
	//var c = require("./delaunay.js");
	...
	//var computeVoronoi = require("./voronoi.js")
	...
	//var triangles;
	//var points_to_triangulate = new Array();
	...
	function animate(){
		...
		// complete points for triangulation
		var i,j;
		
		//points_to_triangulate[0] = [agent_radius,agent_radius];
		//points_to_triangulate[1] = [agent_radius,height-agent_radius];//this might be the other way round
		//points_to_triangulate[2] = [width-agent_radius,agent_radius];//this might be the other way round
		//points_to_triangulate[3] = [width-agent_radius,height-agent_radius];
		//var idx = 4;
		//for(i=0;i<index_polygons;i++){
		//	for(j=0;j<polygons[i].length;j++){
		//		points_to_triangulate[idx++] = enlarged_obstacles[i][j];
		//	}
		//}
		//triangles = Delaunay.triangulate(points_to_triangulate);
	...
	function drawTriangulation(){
        ...
        --i; context.moveTo(points_to_triangulate [triangles[i]][0], points_to_triangulate [triangles[i]][1]);
        --i; context.lineTo(points_to_triangulate [triangles[i]][0], points_to_triangulate [triangles[i]][1]);
        --i; context.lineTo(points_to_triangulate [triangles[i]][0], points_to_triangulate [triangles[i]][1]);
        context.closePath();
        context.stroke();
      }
...
function getNormalVector(vector){
	...
	return [-vector[1],vector[0]]; // !!! this might be the other way round
}
...
function motionPlanning(agent_point_current, target_point, obstacles, width, height){
	...
	while(path_index<10){//(next_point[0]!=target_point[0] && next_point[1]!=target_point[1]){
		...
		for(i = 0; i < obstacles.length; i++){ // increment obstacles
			...
			if(i == current_i){
				...
				if(current_j == 0){
					...
					if(d_closest > d_current){
						d_closest = d_current
						//next_point[0] = obstacles[i][l-1][0];
						//next_point[1] = obstacles[i][l-1][1];
						closest_i = i;
						closest_j = l-1;
					...
					console.log(i)
					console.log("j")
					console.log(current_j-1)
					if(d_closest > d_current){
						d_closest = d_current
						//next_point[0] = obstacles[i][current_j-1][0];
						//next_point[1] = obstacles[i][current_j-1][1];
						closest_i = i;
				...
				d_current = d(agent_point_current,obstacles[i][(current_j+1)%l]) + d(obstacles[i][(current_j+1)%l],target_point)
					...
					console.log(i)
					console.log("j")
					console.log((current_j+1)%l)
				if(d_closest > d_current){
						d_closest = d_current
						//next_point[0] = obstacles[i][(j+1)%l][0];
						//next_point[1] = obstacles[i][(j+1)%l][1];	
						closest_i = i;
			...
			for(j = 0; j < l; j++){ // increment nodes
				...
				if(isPointVisible(agent_point_current, obstacles[i][j],obstacles)){
					...
					console.log(d_current);
					console.log(d_closest);
					if(d_current < d_closest){
						console.log("here")
						console.log(next_point);
						d_closest = d_current;
						//next_point[0] = obstacles[i][j][0];
						//next_point[1] = obstacles[i][j][1];
						//console.log(next_point);
						closest_i = i;
						closest_j = j;
		...
		//path[path_index] = [next_point[0],next_point[1]];
		//console.log(path)
		current_i = closest_i
		current_j = closest_j
		path_index++;
		...
		// do the same from the selected point
	...
	//--------COMMENT THIS!!!!!!!!!-------------------------
	//var path = [[100,100],[200,100],[400,200],[500,300]];
...
function isPointVisible(agent_point_current, current_target_point, obstacles){
		...
		//console.log(current_target_point)
		for(i = 0; i < obstacles.length; i++){ // increment obstacles
			var l = obstacles[i].length;
			for(j = 0; j < l; j++){ // increment nodes
				...
				//console.log(target_point)	
				//console.log(isect);
				//console.log(d(isect,target_point))
